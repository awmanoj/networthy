...
def expenses_income(request: Request, annual_income: str = Form("")):
    """Set or clear annual income by hand."""
    amount = _opt_float(annual_income)
    storage.save_annual_income(request.state.user.id,
                               amount if amount and amount > 0 else None)
    return RedirectResponse(url="/expenses", status_code=303)
