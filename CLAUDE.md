...
- **`app/classify.py`** — a layered, config-driven asset-class rule engine (section context >
  ...
  **`asset_class` is computed at parse time and stored**, so a classifier fix only reaches statements uploaded *after* it — `storage.reclassify_holdings()` (run from `init_db`, idempotent) re-files rows already in the database. It is deliberately **not** a blanket re-classify: NPS is classified from CAS *section* context and the section isn't stored, so re-running the rules blind would see an `INF` prefix and turn NPS holdings into mutual funds. It only touches rows currently `mutual_fund`/`direct_equity` that the current rules call `private_equity`, `gold` or `silver`, and it re-runs with `section=MUTUAL_FUND` so the debt/gilt keywords can't fire on "HDFC Corporate Bond Fund".

  **The section shortcut had a hole worth knowing about.** `classify` returned early on `section is MUTUAL_FUND`, *before* the keyword rules — so an NSDL MF-folio section called a gold fund `mutual_fund` while the CAMS parser (which passes `section=UNKNOWN` on purpose) called the same fund `gold`. Two different leaves, and `merge_sources` only de-duplicates **within** a leaf, so the holding was **counted twice in net worth**. Now `_FUND_REFINING` (`GOLD`/`SILVER`/`PRIVATE_EQUITY`) may override the section context — those *refine* "a fund" rather than contradict it — while debt and gilt keywords still may not, or a corporate-bond fund would stop being a mutual fund. `test_merge_sources.py` pins that both parsers agree on a gold fund, which is the invariant that actually prevents this class of bug. The **decisive signal comes from the statement, not from naming convention**: NSDL writes non-transferability into the security name itself — *"AL Trust - Slang Labs 2-Category I-Slang Labs 2 Close ended - Restricted Transferability"* — and no mutual fund carries it, so `restricted transferab` is the primary rule. SEBI's `Category I/II/III` is the secondary one, and it deliberately does **not** require the word "fund" nearby: the real names have no "fund" in them, which is exactly how the first version of that rule missed them. "Close ended" is deliberately *not* a signal on its own — close-ended mutual funds (FMPs) genuinely exist. A second family of these names has no transferability marker but does carry the SEBI category and a **face value written into the security name** — *"AL TRUST BASIL-CATEGORY-I-CLASS BASIL -FACE VALUE INR 100.0/- AND PAID UP"*. That is a **parser** hazard, not just a classifier one: those digits sit immediately before the real columns, and an unlisted AIF row has only *two* of them (units, value — there's no market price), so the positional read took the face value as units and shifted everything. `_mask_face_values` hides the digits inside `[FACE ]VALUE INR <n>` from the column scan using a **same-length** substitution so every index still lines up; the name is sliced from the original text and keeps its real wording. `test_parser.py` pins all five real names at both two and three columns. Known edge: IPO anchor lock-ins are also restricted; filing those as alternate rather than liquid equity is the lesser error, and the override covers it. **Name patterns alone can't win this** —  most real AIF names carry no marker — "True North Fund VI", "IIFL Special Opportunities Fund Series 7" read exactly like mutual funds — so `holding_overrides` is the escape hatch. A user re-files a holding from the leaf page (`POST /networth/holding/reclassify`, `RECLASS_CHOICES`), remembered against the **ISIN, or the name when there is none** (`storage.override_key`) — **not the row id**, because re-uploading a statement rebuilds `holdings` wholesale and an id-keyed override would silently vanish at exactly the wrong moment. `storage.apply_overrides` runs in the read path *before* the asset-class filter (so both `list_networth_holdings` and `latest_holdings_by_class` now filter in Python, not SQL), which is what lets a re-filed row land in a different leaf; it's in `EXPORT_TABLES` too. `test_aif.py` pins the routing, the double-count, the manual override surviving a re-upload, and the migration (including that NPS survives it); the AIF keyword rules sit **before** the
...
- **`GET /net-worth-tracker-excel`** (`main.net_worth_tracker_excel`, template
  `tracker_excel.html`, `XLSX_PATH`) + **`app/static/networth-tracker.xlsx`** — a free
  net-worth tracker spreadsheet and the page that gives it away. Aimed at "net worth tracker
  Excel": that searcher wants *this product* and is settling for a spreadsheet because they
  don't know it exists, so the honest way to meet them is a genuinely good spreadsheet. The
  page's second half argues for not using one — and **opens by conceding a spreadsheet is
  fine**, because the argument is far more persuasive from someone who just handed you the
  file, and overstating it would be both dishonest and less effective.
  - The `.xlsx` is a **committed build artefact**, generated by `scripts/build_tracker_xlsx.py`.
    **openpyxl is a build-time dependency only** — deliberately *not* in `requirements.txt` or
    `pyproject.toml`, because the app never touches xlsx at runtime. Re-run the script when
    the categories change.
  - Categories come from the **same `_CALC_ASSETS`/`_CALC_LIABILITIES` slugs the web
    calculator uses**, so the spreadsheet, the calculator and the tree are one list;
    `test_tracker_xlsx.py` fails if they drift. It also pins that totals/net worth are
    **formulas** (the promise is "you only ever type values"), that the date row
    **self-populates** via `EDATE` so a downloaded file doesn't go stale, that the rupee
    format uses **Indian digit grouping** (Excel has no locale for it — the format string is
    a three-branch conditional for crore/lakh/plain), that the chart exists, and that the
